...
class WebPage:
    ...
    def open(self, wait_condition = None, time = 0, timeout_msg = ''):
        self.app.openPage(self.page_url, wait_condition, time, timeout_msg)
        return self

    def __getattr__(self, name):
        method = getattr(self.app, name, None)
        if(method is None): raise AttributeError('The method "{0}" was not found in the class "{1}"'.format(name, self.app.__class__))
        return method